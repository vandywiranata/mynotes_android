...
public class TaskDBHelper extends SQLiteOpenHelper {
    ...
    public static final String DATABASE_NAME = "MyNotesDBHelper.db";
    public static final String CONTACTS_TABLE_NAME = "MyNotes";
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        db.execSQL(
                "CREATE TABLE "+CONTACTS_TABLE_NAME +
                        "(id INTEGER PRIMARY KEY, notes TEXT, dateStr INTEGER)"
        );
    }
    ...
    public boolean insertContact  (String notes, String dateStr)
    {
        Date date;
    ...
    public boolean updateContact (String id, String notes, String dateStr)
    {
        SQLiteDatabase db = this.getWritableDatabase();
        ContentValues contentValues = new ContentValues();
    ...
    public Cursor getDataYesterday(){
        ...
        Cursor res =  db.rawQuery("select * from "+CONTACTS_TABLE_NAME+
                " WHERE date(datetime(dateStr / 1000 , 'unixepoch', 'localtime')) < date('now', '-0 day', 'localtime')  order by dateStr asc", null);
        return res;
    ...
    public Cursor getDataUpcoming(){
        SQLiteDatabase db = this.getReadableDatabase();
        Cursor res =  db.rawQuery("select * from "+CONTACTS_TABLE_NAME+
                " WHERE date(datetime(dateStr / 1000 , 'unixepoch', 'localtime')) > date('now', '+1 day', 'localtime') order by dateStr asc", null);
        return res;
    ...
    public long deleteData(int id) {
        SQLiteDatabase db = this.getReadableDatabase();
        return db.delete(CONTACTS_TABLE_NAME, "id = " + id, null);
    }
}
